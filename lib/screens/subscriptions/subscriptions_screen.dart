...
class _PaymentFormScreenState extends State<_PaymentFormScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      const SizedBox(height: 12),
      TextFormField(controller: _amountCtrl, decoration: const InputDecoration(labelText: 'قيمة الاشتراك (أوقية)'), keyboardType: TextInputType.number, validator: (v) => (double.tryParse(v ?? '') ?? 0) <= 0 ? 'أدخل قيمة صحيحة' : null),
      const SizedBox(height: 12),
      ...
      const SizedBox(height: 12),
      Row(children: [Expanded(child: DropdownButtonFormField<int>(value: _year, decoration: const InputDecoration(labelText: 'السنة'), items: [for (var y = DateTime.now().year - 5; y <= DateTime.now().year + 1; y++) DropdownMenuItem(value: y, child: Text('$y'))], onChanged: (v) => setState(() => _year = v ?? _year))), const SizedBox(width: 8), Expanded(child: DropdownButtonFormField<int>(value: _month, decoration: const InputDecoration(labelText: 'الشهر'), items: [for (var m = 1; m <= 12; m++) DropdownMenuItem(value: m, child: Text('$m'))], onChanged: (v) => setState(() => _month = v ?? _month))) ]),
      const SizedBox(height: 12),
