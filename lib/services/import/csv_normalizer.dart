/// lib/services/import/csv_normalizer.dart
...
class CsvNormalizer {
  ...
  static const Map<String, String> _headerMap = {
    ...
    'subscription': 'amount',
    'amount': 'amount',
    'رسوم البطاقة': 'card_fee',
    'card fee': 'card_fee',
    'card_fee': 'card_fee',
    'طريقة الدفع': 'payment_method',
    'payment method': 'payment_method',
    'تفاصيل الاشتراك': 'details',
    'تفاصيل': 'details',
    'details': 'details',
    'دفع مباشر للتنفيذي': 'direct_exec',
    'direct executive': 'direct_exec',
    'صفحة المصدر': 'source_page',
  ...
  static String headerKey(String header) {
    return _headerMap[norm(header)] ?? '';
  }

  static String memberHeaderKey(String header) {
    return _memberHeaderMap[norm(header)] ?? '';
  }
  ...
  static List<int> extractMonths(String details) {
    ...
    _monthNames.forEach((name, number) {
      if (d.contains(norm(name))) {
        months.add(number);
      }
    });

    final matches =
        RegExp(r'شهر\s*(1[0-2]|[1-9])').allMatches(d);
  ...
  static double cardFeeFromDetails(
    String details,
    double total,
    double configuredCard,
  ) {
    final card =
        configuredCard <= 0 ? 200.0 : configuredCard;

    final d = norm(details);

    if (d.contains(norm('بطاقة')) && total >= card) {
      return card;
    }
  ...
  static int subscriptionMonthCount(
    ...
    if (d.contains(norm('سنة كاملة')) ||
        d.contains(norm('سنه كامله'))) {
      return 12;
    }
    ...
    if (d.contains(norm('بقية السنة')) ||
        d.contains(norm('بقيه السنه'))) {
      return subscriptionAmount > 0
          ? (subscriptionAmount / m)
              .round()
              .clamp(1, 999)
          : 1;
    }
    ...
    for (final entry in words.entries) {
      if (d.contains(norm(entry.key))) {
        return entry.value;
      }
    }

    return subscriptionAmount > 0
        ? (subscriptionAmount / m).round()
        : 0;
  }
}
